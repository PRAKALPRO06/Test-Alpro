...
func InputData(nama_2311102013 *NamaProv, pop *PopProv, tumbuh *TumbuhProv, jumlah *int) {
	...
	for i := 0; i < *jumlah; i++ {
		...
		fmt.Scanln(&nama_2311102013[i])
		fmt.Printf("Masukkan populasi provinsi %s: ", nama_2311102013[i])
		fmt.Scanln(&pop[i])
		fmt.Printf("Masukkan angka pertumbuhan penduduk provinsi %s: ", nama_2311102013[i])
		fmt.Scanln(&tumbuh[i])
	}
}
...
func main() {
	...
	// Input data
	InputData(&nama, &pop, &tumbuh, &jumlah)
	if jumlah <= 0 {
